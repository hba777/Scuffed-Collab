...
class MyDateUtil{
  ...
  //Get Formatted Date from MilliSecondsSinceEpochs
  static String getFormattedDateTime({required BuildContext context, required String time}) {
    // Convert the string to millisecondsSinceEpoch and then to DateTime
    final date = DateTime.fromMillisecondsSinceEpoch(int.parse(time));

    // Format the date using intl package
    final formattedDate = DateFormat('dd-MM-yyyy').format(date);

    // Format the time using TimeOfDay
    final formattedTime = TimeOfDay.fromDateTime(date).format(context);

    // Return a combined formatted date and time
    return formattedDate; // Output example: "2024-10-18 14:30"
  }
